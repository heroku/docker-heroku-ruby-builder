...
Publishing is guarded against duplicates: `--status published` first scans published entries from roughly the last four years (wide enough to cover every supported version) and, if one already has the same title and published state, reports it and exits successfully (status 0) without creating a duplicate — a matching entry is treated as an idempotent no-op, not an error, so re-running a build for an already-announced version leaves the existing entry untouched. Content is intentionally not compared, so a regenerated body cannot slip a second entry past the guard. Drafts are never deduplicated, so `--status draft` always creates an entry (a convenient check that the API and token work).
